...
public class UserController {
	...
	public String processFriends(@ModelAttribute Friends friends, @RequestParam("fImage") MultipartFile f, Principal p,
			HttpSession session) {
		try {
	...
	// showing specific friend on click
	...
	public String showdetails(@PathVariable("fId") Integer fId, Model model, Principal p) {
		...
		if (u.getId() == friends.getUser().getId())
			; // if id from user table is equal id of friends table than it will run
		{
	...
	public String processupdate(@ModelAttribute Friends friends, @RequestParam("fImage") MultipartFile updtfile,
			Model m, HttpSession ses, Principal p) {
		try {
			// old friends detail to be fetch
